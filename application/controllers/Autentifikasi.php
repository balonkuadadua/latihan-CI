...
class Autentifikasi extends CI_Controller
{
    ...
    private function _login()
    {
        ...
        if ($user) {
            ...
            if ($user['is_active'] == 1) {
                //cek password
                if (password_verifiy($password, $user['password'])) {
                    $data = [
                        'email' => $user['email'],
                ...
                $this->session->set_flashdata('pesan','<div class="alert alert-info alert-message" role="alert">User Belum diaktifasi! </div>');
                redirect('autentifikasi');
            }
        } else {
